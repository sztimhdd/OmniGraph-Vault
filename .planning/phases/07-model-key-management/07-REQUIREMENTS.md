# Phase 7 — Model & Key Management

> **Status:** Requirements locked. Research complete — see [`07-RESEARCH.md`](./07-RESEARCH.md) for verified implementation specifics.
> **Goal:** Replace scattered `GEMINI_API_KEY` reads, hardcoded model strings, and one-off rate limiters with a single shared `lib/` module — while staying compatible with Hermes and OpenClaw skill conventions.

---

## Research-Applied Corrections (2026-04-28)

The following corrections supersede the text below. Refer to [`07-RESEARCH.md`](./07-RESEARCH.md) for evidence:

| #  | Correction | Source |
| -- | ---------- | ------ |
| R1 | **Migration scope is 25 files**, not ~18 as estimated in §5. See Migration Scope Map in research doc. | Fresh grep at research time |
| R2 | **Add `lib/cognee_bridge.py`** to the module layout in §4. Cognee's `@lru_cache`-decorated config means env rotation alone doesn't reach it — requires direct singleton mutation via a rotation listener. | `cognee.infrastructure.llm.config:271` |
| R3 | **Canonical `INGESTION_LLM` is `gemini-2.5-flash-lite`** (GA, 15 RPM free). NOT `gemini-3.1-flash-lite-preview` (preview, no free tier, may disappear). `ingest_github.py` and `config.py` defaults must change. | Research §Common Pitfalls |
| R4 | **No SDK migration needed.** All 19 production files already use new `google-genai` SDK; zero use deprecated `google-generativeai`. Phase 7 stays a pure library-add. | Grep verified |
| R5 | **Dependencies `aiolimiter 1.2.1` + `tenacity 9.1.4` are already installed** as transitive deps via Cognee/LightRAG. Adding to `requirements.txt` is for reproducibility, not new installs. | `venv/Lib/site-packages/*.dist-info/METADATA` |
| R6 | **LightRAG has a latent interop bug** (`lightrag/llm/gemini.py:42` imports deprecated SDK exception class while using new SDK client). Our `lib.generate` wraps LightRAG calls as the authoritative retry layer. | LightRAG source lines 40–42 |
| R7 | **`AsyncLimiter` + `@retry` nesting verified:** `@retry` OUTSIDE, `async with _limiter` INSIDE. Correct as written in §2.4. | `aiolimiter/leakybucket.py:197-203` |
| R8 | **`APIError.code` is a verified int** in google-genai 1.73.1. `_is_retriable` predicate is correct as written. | `google/genai/errors.py:33,46,64` |
