...
**Sub-incident (discovered 2026-05-02 by GSD:quick session during Cognee fix)**: commit `0dc4b2b` (2026-05-02 02:58) had silently NULLed `_resolve_model()`'s Vertex-mode `gemini-embedding-2 → -preview` mapping with a comment "gemini-embedding-2-preview deprecated by Vertex AI". Empirical probe on 2026-05-02 disproved this — `gemini-embedding-2` still returned Vertex 404 NOT_FOUND; `-preview` was still the working name. The ~17-hour window between `0dc4b2b` and `8e4b132` left the LightRAG Vertex path silently broken for anyone who actually exercised it (Hermes's P0 re-ingest apparently tolerated it because the embedding init path hit a different resolution cache). Restored by GSD:quick in `8e4b132`. **Recorded as operational lesson**: any change to `_resolve_model()` or model-name constants should require an automated smoke-probe against real provider endpoints, not just visual review of a comment. Filed as a v3.3 hygiene item.

**Follow-up 2026-05-03 (commit `9069f59` + `ae3a030`)**: Hermes hit 404s on every Vertex embedding call the morning of 2026-05-03. Empirical probe against us-central1 with the paid-tier SA this time showed the **opposite** of 2026-05-02: `gemini-embedding-2-preview` now returns 404; **`gemini-embedding-2` returns 3072-dim vectors directly**. Google removed `-preview` from the Vertex catalog sometime between the 2026-05-02 probe and the 2026-05-03 morning window. Hermes shipped `9069f59` making `_resolve_model()` a pass-through. Two existing test files (`test_cognee_vertex_model_name.py`, `test_lightrag_embedding_vertex.py`) still carried `-preview` assertions and were latently broken on main until `ae3a030` realigned them. **Refined operational lesson**: the 2026-05-02 "v3.3 hygiene item" is upgraded from nice-to-have to near-term — we have now seen the Vertex model-name contract flip twice in 24 hours. A scheduled live-probe (cron or pre-deploy hook) against `embed_content(model="gemini-embedding-2")` is the minimum cost vs catching this via in-batch 404 storms. Separately: the parallel-agent framing that appeared locally on 2026-05-03 ("migration deferred, stay on Gemini free tier") was **wrong** — Hermes is already on Vertex AI paid and it works; that framing was discarded and did not land on main.

**Follow-up 2026-05-03 PM — THIRD flip (commit `99a1fb8`)**: Phase 18-00 HYG-01 `vertex_live_probe.py` shipped `5dc13f9`; Hermes ran it as part of Wave 1 verification and caught a FOURTH Vertex catalog change within 4 days: `gemini-embedding-2` now 404s again, `gemini-embedding-2-preview` is back and returns 3072-dim. The pass-through from `9069f59` is now wrong; commit `99a1fb8` reinstates the `-preview` alias via a centralized `_VERTEX_EMBEDDING_ALIAS` dict. Root cause per the Google Cloud "Model versions and lifecycle" docs: `gemini-embedding-2` is PREVIEW-lifecycle (not Stable); Google is actively attempting preview→stable promotion and rolling back. LiteLLM issue #23322 corroborates independently. **Until Google officially GAs the model on the Stable Models page, `-preview` is canonical**. The 18-00 probe proved its worth: instead of discovering the flip via tomorrow 06:00 ADT's cron failure storm, we caught it during same-day verification — ~12 hours of lead time. The "v3.3 hygiene" upgrade from near-term to **active** is now actioned by 18-00's monthly probe (consider bumping to weekly given we've seen 3 flips in 4 days). Follow-up Phase 18-00 v2 concept: runtime fallback chain in `_resolve_model` so the probe-to-patch loop self-heals without a commit.
