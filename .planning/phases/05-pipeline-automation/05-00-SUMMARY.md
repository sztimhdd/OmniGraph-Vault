...
### § C narrative — endpoint × model mismatch (retroactively corrected 2026-05-03 PM)

**Lesson: When Google releases a new model, probe the full endpoint matrix, not a single endpoint.**

The earlier framing in this section (multiple same-week Google catalog changes) was an illusion. Google did not change the Vertex catalog during this window; we were looking at two different endpoints and mistaking endpoint × model coverage for temporal instability. Corrected account:

- **2026-04-22:** Google promoted `gemini-embedding-2` to GA on the Vertex AI `global` endpoint. Regional endpoints (`us-central1` et al.) did NOT receive the unsuffixed GA alias; they continue to expose `gemini-embedding-2-preview` instead. This was a documented Google release, not a flip.

- **2026-04-30 → 05-03 (observed confusion, not actual Google flips):** A cluster of compounding illusions trapped the project into re-patching the same code three times:
  - Our `scripts/vertex_live_probe.py` ran in `us-central1`, where `-preview` worked and unsuffixed `-2` did not. Probe output read as "unsuffixed is broken today".
  - `lib/lightrag_embedding._VERTEX_EMBEDDING_ALIAS` silently remapped every `-2` request to `-preview` regardless of endpoint. That made the regional-mode behavior leak into production.
  - Production Hermes `.env` had (undocumented at the time) adopted `GOOGLE_CLOUD_LOCATION=global` to pool quota across projects. At `global`, `-preview` does not exist, so the silent alias caused every embed to 404.
  - The three in-repo "fix" commits during this window (`0dc4b2b` 2026-05-02, `9069f59` 2026-05-03 AM, `99a1fb8` 2026-05-03 PM) each moved the alias one way or the other under the assumption that Google was toggling the catalog. Each one looked correct against the regional-probe output; each was wrong against the global-production path.
  - Net effect: probe stable / production 404s every embed / 2-machine divergence. The "flip" narrative was an artifact of probing one endpoint while production ran the other.

- **2026-05-03 evening:** Hermes caught the production 404 cascade end-to-end (RSS ingest appeared as a LightRAG timeout on retry to deadline). Claude verified against Google Cloud docs and `gemini-cli` issues #19055 + #20761 that the endpoint × model-name mapping is static and documented: `global` hosts the GA alias, regional endpoints host `-preview`. Nothing had been flipping.

- **Fix (this hygiene commit, quick-260503-m4q):** delete `_VERTEX_EMBEDDING_ALIAS` and `_resolve_model()`; pass `gemini-embedding-2` through as-is; upgrade `scripts/vertex_live_probe.py` to a 2×3 matrix over (`global`, `us-central1`) × (`-2`, `-2-preview`, `-001`) with a `known_good` expectation table so future endpoint-name misses are caught by the probe itself rather than by production.

The 2026-04-30 / 2026-05-02 / 2026-05-03 timestamps are preserved as historical record — they reflect when the confusion surfaced, NOT when Google changed anything.
