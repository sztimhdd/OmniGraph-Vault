...
| **TOTAL** | **+218 added / −27 removed = +191 net; gross changed = 218** | Orchestrator decision Z accepted (waiver still applies; +191 net replaces +201 after T5 split per plan-checker rec #1). |

**Right-Size justification (orchestrator-waived):** Halt Trigger "LoC > 150" was triggered. User selected option Z — accept A dispatcher design at +201 net LoC. This phase explicitly remains plan-phase tier (multi-subsystem coordination: lib/ dispatcher + databricks-deploy/ helper + kb/api.py wrapper replacement + 2 test layers + 1 deploy artifact env update). The eval harness alone (T4 +50) is plan-phase mandate per Principle #8 ("measurement infrastructure"). No quick downshift possible.
...
<task id="P2-3-perf-fix-A-T6" wave="2" depends_on="P2-3-perf-fix-A-T5" autonomous="false" requirements="SC#1,SC#2,SC#3,SC#4,SC#5,SC#6">
  ...
  <action>
    ...
    **Write `P2-3-perf-fix-A-VERIFICATION.md`** at `.planning/phases/v1.1-roadmap/P2-3-perf-fix-A/`:
    ...
    - **LoC waive log (orchestrator decision Z):** +201 net LoC at plan-phase ceiling boundary; user explicitly waived 150 ceiling on 2026-05-31. Phase remains plan-phase, not escalated.
