...
| P2-3 | 2 | ⚠️ DEPLOYED-DISABLED 2026-05-31 | BGE-v2-m3 reranker + `mix` mode (paired). 7 P2-3 commits + escape `b4f52c5`. **SC#2 deferred** — pre-escape 230s+ FAIL (BGE rerank on N=131 chunks at CPU = 160s > 150s inner timeout, RESEARCH §2 N=20 assumption 6.5× miss). Operational Escape `BGE_FORCE_LOAD_FAIL=1` in `app.yaml`: post-escape **62.59s PASS** (mode='hybrid' fallback, P5 baseline). SC#1 ✅ (~29s lifespan), SC#3 ❌ NOT MEASURABLE, SC#4 ✅ via escape, SC#5 ✅ STRICT. Databricks deployment `01f15c8a8b0d163382ebce4d3be8b028` RUNNING. Follow-up: `v1.1.P2-3-perf-fix-A` (LLM-as-reranker via Databricks Haiku batch JSON). Code retained in git, runtime disabled until fix ships. See [P2-3-VERIFICATION.md](P2-3/P2-3-VERIFICATION.md) + [P2-3/PLAN.md](P2-3/PLAN.md) + [P2-3/RESEARCH.md](P2-3/RESEARCH.md). |
| P2-3-perf-fix-A | 2 | 📋 PLANNED 2026-05-31 | LLM-as-reranker (Databricks Haiku 4.5 batch JSON) — replaces P2-3 BGE wrapper, eliminates CPU rerank latency root cause that triggered escape. **PLAN+RESEARCH committed 2026-05-31**, plan-checker FLAG (PASS w/ 6 recs; rec #1 fixed inline at T5). +191 net LoC (orchestrator-waived ceiling 150 → option Z). Aliyun parity DEFERRED to phase B. SC: cold-start ≤60s, p50 ≤65s, token-overlap ≥+10%, graceful-degrade dual-env (`OMNIGRAPH_LLM_RERANK_FORCE_FAIL=1` + legacy `BGE_FORCE_LOAD_FAIL=1` compat), kb/static+templates untouched, P2-3 escape preserved. See [P2-3-perf-fix-A/PLAN.md](P2-3-perf-fix-A/PLAN.md) + [P2-3-perf-fix-A/RESEARCH.md](P2-3-perf-fix-A/RESEARCH.md). |
| P2-3-perf-fix-B | 2 | 📋 QUEUED 2026-05-31 | Aliyun Vertex Gemini batch JSON rerank parity — adds `lib/vertex_gemini_rerank.py` + `lib/llm_rerank.py` vertex route + Aliyun systemd env + deploy + smoke. ~+65 LoC. **Blocked on A close.** Until B ships, Aliyun retains P5 baseline mode='hybrid'. ISSUES.md row #22. |
