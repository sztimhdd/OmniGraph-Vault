...
| P2-3 | 2 | ⚠️ DEPLOYED-DISABLED 2026-05-31 | BGE-v2-m3 reranker + `mix` mode (paired). 7 P2-3 commits + escape `b4f52c5`. **SC#2 deferred** — pre-escape 230s+ FAIL (BGE rerank on N=131 chunks at CPU = 160s > 150s inner timeout, RESEARCH §2 N=20 assumption 6.5× miss). Operational Escape `BGE_FORCE_LOAD_FAIL=1` in `app.yaml`: post-escape **62.59s PASS** (mode='hybrid' fallback, P5 baseline). SC#1 ✅ (~29s lifespan), SC#3 ❌ NOT MEASURABLE, SC#4 ✅ via escape, SC#5 ✅ STRICT. Databricks deployment `01f15c8a8b0d163382ebce4d3be8b028` RUNNING. Follow-up: `260531-bge-rerank-perf-fix` (chunk truncation / batch_size / FP16). Code retained in git, runtime disabled until fix ships. See [P2-3-VERIFICATION.md](P2-3/P2-3-VERIFICATION.md) + [P2-3/PLAN.md](P2-3/PLAN.md) + [P2-3/RESEARCH.md](P2-3/RESEARCH.md). |
