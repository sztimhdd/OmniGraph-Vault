...
2. **Provider keys + env 部署到阿里云**
   - 扩展现有 `/root/.hermes/.env` (kb-api 已用),append ingest 侧 keys;不创建独立 `/etc/omnigraph/.env`(决策依据 STATE-Aliyun-Ingest-Migration-v1.md:132-133)
   - DeepSeek API key + SiliconFlow API key + Vertex SA JSON

3. **Code 部署到阿里云**
   - 复用 kb-api 现有 checkout `/root/OmniGraph-Vault/`(in-place,不再 clone 第二份 tree 到 `/opt/omnigraph-vault/`);pre-aim-1 dirty 工作树由 operator reconcile 到一个 known HEAD
   - venv 落在 `/root/OmniGraph-Vault/venv/`(若 kb-api 已用同路径 venv 则验证 Python 3.11+ 后复用);`pip install -r requirements.txt`
