...
- **Cognee restoration into `kg_synthesize`** (interim removal 2026-05-02 per `0109c02`, Vertex model-name mismatch fixed by GSD:quick `8e4b132`). **Decision 2026-05-02: do NOT auto-restore.** The model-name fix only addresses one of the two root causes Hermes recorded in `0109c02`; the second (Cognee module-level import triggers async pipelines that block the event loop) is unverified post-fix — it may have been a 404-cascade side-effect OR a separate Cognee design issue. Restoration requires its own small plan: (1) smoke test that `import cognee` no longer blocks; (2) lazy-import pattern (function-local, not module-level); (3) `asyncio.wait_for(timeout=5)` around recall/remember; (4) prompt-integration design for injecting past_context without regressing the 2/2 image inline rate; (5) P2-style end-to-end validation. Trigger: user reports concrete "synthesis forgets prior queries" pain. Until then, ingest-side Cognee (`remember_article`, `cognee_batch_processor`) is the load-bearing path and works correctly post-`8e4b132`; the query-side memory layer is a nice-to-have, not a gate.
