...
- [ ] **DEPLOY-01**: Code deployed to Aliyun in-place at `/root/OmniGraph-Vault/` (sharing kb-api's existing checkout per STATE-Aliyun-Ingest-Migration-v1.md:132-133 path correction; do NOT clone a second tree at `/opt/omnigraph-vault/`). Pre-aim-1 dirty working tree (per Gate 1 SSH probe) reconciled to a known HEAD by operator (commit / stash / discard per operator judgement) and rationale recorded in `.planning/phases/aim-1-*/DEPLOY-NOTES.md`. Working tree clean (`git status` empty) post-reconcile. HEAD commit hash recorded in deploy notes. Repo origin = `git@github.com:sztimhdd/OmniGraph-Vault.git` (or operator's fork).
- [ ] **DEPLOY-02**: Python venv created at `/root/OmniGraph-Vault/venv/` with Python 3.11+ (do NOT collide with kb-api's existing venv if it lives elsewhere; if the kb-api venv is already at this path, reuse + verify Python 3.11+ + run `pip install -r requirements.txt` to converge to ingest deps). `pip install -r requirements.txt` succeeds with zero errors. `python -c "import lightrag, google.genai, deepseek; print('OK')"` (or equivalent import smoke) prints OK.
- [ ] **DEPLOY-03**: Provider credentials placed on Aliyun ECS by appending ingest-side keys to existing `/root/.hermes/.env` (do NOT create separate `/etc/omnigraph/.env` — kb-api already reads `/root/.hermes/.env`; a second env file fragments configuration and creates dual-source-of-truth drift). Required keys after append: `DEEPSEEK_API_KEY` (real or `dummy` per Phase 5 cross-coupling), `SILICONFLOW_API_KEY`, `OMNIGRAPH_VERTEX_SA_JSON_PATH` (Vertex SA JSON path), `GEMINI_API_KEY` (legacy fallback), `APIFY_TOKEN` + `APIFY_TOKEN_BACKUP`. Preserve existing file mode + ownership (do not chmod 600 if existing mode differs — match the file's pre-aim-1 state). Preserve all pre-existing kb-api keys unchanged (additive append only). No literal secrets committed to repo or any planning doc.
