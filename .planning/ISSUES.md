...
**Created:** 2026-05-30
**Last updated:** 2026-06-01 (260601-ipo Aliyun ingest OOM mitigation IN FLIGHT — #4 marked stale + replaced by #25 RAM-mitigation entry; commit 91b33f1 deployed) + perf-fix-B HT-4 halt — reconcile follow-up filed (#26)
...
| 25 | **Aliyun all-in-one + LightRAG nano-vectordb full-load is the structural OOM root cause.** 260601-ipo capped peak via systemd `MemoryMax=4G` + `Conflicts=` mutex + LightRAG concurrency 4→2 + `gc.collect()` per article. This is band-aid; treats symptom not root. **Real fix: LightRAG → Qdrant migration** (user wrote design doc, separate phase). After migration the full-vdb load that drives 3 GB baseline RSS goes away — Qdrant serves vectors over network, ingest worker holds only the working set. | `v1.1-qdrant-migration` (user-owned doc) | Filed 2026-06-01 during 260601-ipo close-out. P3 / future-scope; trigger = OOM resurfacing at 4G cap (would mean cap has to grow OR Qdrant migration must ship). |
