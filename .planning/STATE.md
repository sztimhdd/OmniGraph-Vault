...
Last activity: 2026-05-10 - Completed quick task 260510-h09: hot-fix inner `ingest_article` PROCESSED-gate to RAISE `RuntimeError` (was silent log+skip) + retry-with-backoff helper (`PROCESSED_VERIFY_MAX_RETRIES=3`, `PROCESSED_VERIFY_BACKOFF_S=2.0`); inner INSERT updated to include `source='wechat'` (mig 008 schema). Outer `batch_ingest_from_spider.ingest_article` now marks `ingestions.status='failed'` when verification fails so mig 009 retry pool re-queues next cron — closes the 2026-05-09/10 ainsert async-pipeline race (4 ingestions=ok wechat / 0 LightRAG status='processed' on cron 09:00 ADT 2026-05-10). Path A hot-fix scope; Path B/C deferred. Prior 260510-gqu investigation locked the root cause; this h09 quick implements the in-scope hot-fix surface (caller-side PROCESSED gate + status='failed' propagation).
