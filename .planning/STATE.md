...
Last activity: 2026-05-05 — Completed quick task 260505-sjk: closed audit ece03ae Mismatch #1 (🔴). UA fallback path now merges `result["img_urls"]` (images outside `#js_content`) with `process_content(content_html)` output, mirroring legacy `ingest_article:978` semantics. Patch is 7-line consumer-only diff in `lib/scraper.py:195-200`; UA function untouched. 4 mock-only tests GREEN; 5/5 regression tests in `test_scraper.py` still GREEN. Mismatches #2 (🟡 Apify markdown regex) + #3 (🟡 CDP body-fallback observability) DEFERRED per audit recommendation
