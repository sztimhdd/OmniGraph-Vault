...
| Feature | Why Expected | Complexity | Notes |
|---------|--------------|------------|-------|
| 50+ GitHub AI tools indexed in LightRAG | `omnigraph_query` must return useful answers to "what is LangChain?" before `/architect` query mode can work | MEDIUM | Batch ingest via Graphify MCP or manual loop; creates `entity_registry.json` as side effect |
...
| Web scraping in `/architect` ingest mode (arbitrary URLs, not just WeChat) | Users want to add GitHub READMEs, blog posts, Hacker News threads directly | `ingest_wechat.py` is purpose-built for WeChat; arbitrary URL scraping requires Firecrawl or a new code path; adds a new dependency and new failure modes | Use Graphify MCP for batch GitHub tool ingestion (Task 2.1-03); use the existing `ingest_wechat.py` for WeChat + PDF only; document the limitation clearly in the skill |
...
| KB population complexity | MEDIUM | Task 2.1-03 mentions "Graphify MCP" which is not confirmed available; complexity estimate assumes Graphify works as described; if unavailable, batch ingest falls back to manual loop (higher time cost, same end state) |
