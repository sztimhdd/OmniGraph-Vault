...
| NLP | 1013 | 1013 | 1056 → 1056 ⚠️ deferred |

**Fix validated end-to-end:** Agent/LLM/RAG climbed 1013 → 2069 (full CV parity), proving the multi-topic loop now iterates every topic. Before the fix, a 5×`--topic` invocation only ever classified CV.

**Candidate-pool unlock:** 1916 distinct `relevant=1` articles now flagged across the 4 previously-starved topics (Agent=1625, LLM=1879, RAG=1473, NLP=877 relevant). These are ingest candidates; `articles.layer2_verdict='ok'` (currently 473) grows as the every-2h ingest cron processes the newly-unlocked pool.

**NLP deferred (transient, not this bug):** NLP aborted on batch 1 across 7 attempts over ~40 min with `DeepSeek API call failed: Expecting value: line 1 column 1 (char 0)` (empty API response). RAG/LLM/Agent/CV all succeeded on the *identical* 1056-article set — so this is a DeepSeek-path flake, orthogonal to the argparse fix. The script aborts a whole topic on any single batch failure (no in-script retry). Logged to ISSUES.md as a new P1. **Auto-resolves:** the systemd `omnigraph-kol-classify.timer` (next fire Fri 2026-06-26 19:15 CST) now correctly runs all 5 topics, so NLP retries automatically.
