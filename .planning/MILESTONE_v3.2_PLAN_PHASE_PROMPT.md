...
Milestone v3.1 — **已关闭 2026-05-01 @ commit `2b38e98`**（26/26 REQs delivered, E2E-02 gate revised <120s → <600s based on Hermes DeepSeek baseline 441s/article）。详见 `docs/MILESTONE_v3.1_CLOSURE.md`。v3.2 直接依赖的 v3.1 产物：
...
- Phase 15（B4 docs）和 Phase 16（B5 Vertex AI 设计）**无代码依赖**，立即可执行
- Phase 12（B1 Checkpoint）v3.1 已关闭，立即可执行（不再 gated on v3.1 gate-passing）
- Phase 13（B2 Cascade）、Phase 14（B3 Fixtures）v3.1 已关闭，立即可执行
- Phase 17（批量超时）依赖 Phase 12 checkpoint API 成型后才能定稿；默认 BATCH_TIMEOUT 28800s（8h，覆盖 56 篇 × 441s Hermes baseline）
