...
def _build_rewrite_prompt(title: str, body: str, src_lang: str) -> str:
    ...
    *src_lang* (no translation). It includes the full CONTEXT.md gate
    ...
    return (
        ...
        f"   Stop the ARTICLE TEXT at the last paragraph of real content —\n"
        f"   but the trailing image-reference block (consecutive lines of the\n"
        f"   form `Image N from article '<title>': http://localhost:8765/...`)\n"
        f"   is NOT boilerplate and is NOT part of that judgment: it MUST be\n"
        f"   copied to the end of the output IN FULL, every line, even though\n"
        f"   it sits after the last content paragraph (see rule 7).\n\n"
        ...
        f"   tags — no <script>, <style>, <div>, <span>, and no HTML tables\n"
        f"   (<table>, <tr>, <td>, <th>, <thead>, <tbody>). Convert any HTML\n"
        f"   table into a GitHub-flavored markdown table (| col | col | rows).\n"
        f"   Exception: tag names mentioned AS TEXT inside inline code\n"
        f"   (`<div>`) or fenced code blocks are content, not markup — keep them.\n\n"
        f"7. IMAGE URLs — CRITICAL CONSTRAINT:\n"
        f"   Image URLs beginning with the prefix `http://localhost:8765/` and\n"
        f"   the `![...](...)` markdown around them MUST be reproduced\n"
        f"   BYTE-FOR-BYTE exactly as they appear in the body. NEVER alter,\n"
        f"   shorten, or invent a URL. Treat image lines as opaque tokens — do\n"
        f"   NOT describe or improve images. Also preserve any reference lines\n"
        f"   of the form `Image N from article '<title>': http://localhost:8765/...`\n"
        f"   with the URL byte-identical.\n"
        f"   ONLY reproduce image URLs that ACTUALLY APPEAR in the body below.\n"
        f"   If the body contains NO `http://localhost:8765/` URLs, then output\n"
        f"   MUST contain NONE — do NOT add, invent, or fabricate any image URL.\n"
        f"   Do NOT copy any example/placeholder URL mentioned in these rules.\n"
        f"   Correct handling: an image line already present in the body such as\n"
        f"   `![desc](<one of the body's real localhost:8765 URLs>)` is kept\n"
        f"   unchanged; a real URL is NEVER truncated to `.../0.jpg` and NEVER\n"
        f"   rewritten to a relative path like `./0.jpg`.\n"
        f"   HARD INVARIANT: every DISTINCT `http://localhost:8765/` URL in the\n"
        f"   body MUST appear in your output — dropping even ONE (inline image\n"
        f"   OR trailing `Image N from article` line) makes the output invalid\n"
        f"   and it will be discarded. When unsure whether an image line is\n"
        f"   boilerplate, KEEP it.\n\n"
        ...
        f"   no explanation, no surrounding fences. Use exactly ONE blank line\n"
        f"   between blocks; never emit two or more consecutive blank lines.\n\n"
