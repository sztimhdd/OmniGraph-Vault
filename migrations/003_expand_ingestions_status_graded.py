#!/usr/bin/env python3
"""Apply migration 003: add 'skipped_graded' to ingestions.status CHECK constraint.

Usage:  python3 migrations/003_expand_ingestions_status_graded.py [path/to/kol_scan.db]
...
def migrate(db_path: str) -> bool:
    ...
    schema = row[0]
    if "skipped_graded" in schema:
        print("SKIP: already migrated")
        conn.close()
        return True

    print(f"Migrating {db_path} to add skipped_graded...")
    conn.execute("BEGIN")
    conn.execute("""
