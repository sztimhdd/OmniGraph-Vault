...
2. 字数:1500-3000 字
3. 引用:正文每个论点用 markdown 行内链接格式 [短标签](/article/{{hash}}.html)
   (hash 是文章在知识库中的 10 字符哈希)
   末尾如果列 References / 参考文献 章节,**必须**用 markdown link 格式:
   - [完整文章标题](/article/{{hash}}.html)
...
3. Citations: cite EVERY claim using markdown inline link format [short label](/article/{{hash}}.html)
   ...
   - [Full article title](/article/{{hash}}.html)
...
要求:
1. 回答简洁,200-400 字
2. 每个关键结论用 markdown 行内链接格式 [短标签](/article/{{hash}}.html) 引用具体来源
   (hash 是文章在知识库中的 10 字符哈希)
...
2. Cite key claims using markdown inline link format [short label](/article/{{hash}}.html)
   (hash is the 10-char article hash in the knowledge base).
